...
class RobotArmController:
    def __init__(self, root):
        ...
        self.step_size = 2.0  # Kích thước bước (mm) - càng nhỏ càng mịn
...
Chú ý: Đảm bảo robot đã ở vị trí home trước khi vẽ."""
    ...
    def extract_drawing_path(self, image_path, threshold=128, invert=True, method="contour", detail_level=0.5):
        ...
        for contour in contours:
            ...
            if cv2.contourArea(contour) < 3:  # Giảm kích thước tối thiểu để bắt nhiều chi tiết hơn
                continue
    ...
    def generate_gcode(self):
        ...
        drawing_speed = 1000  # mm/min khi vẽ
    ...
    def move_physical_robot(self, prev_angles, theta1, theta2, pen):
        """Điều khiển robot thực tế với chuyển động mượt mà hơn"""
        if not self.is_connected or not self.arduino:
            return False
        
        try:
            # Tính delta góc chính xác
            delta_theta1 = theta1 - prev_angles[0]
            delta_theta2 = theta2 - prev_angles[1]
            
            # Chia chuyển động thành các bước nhỏ hơn để chuyển động mượt mà
            steps = max(1, int(max(abs(delta_theta1), abs(delta_theta2)) / 10))
            
            for i in range(1, steps + 1):
                # Tính góc trung gian
                intermediate_theta1 = prev_angles[0] + (delta_theta1 * i / steps)
                intermediate_theta2 = prev_angles[1] + (delta_theta2 * i / steps)
                
                # Tính số bước cho động cơ dựa trên góc trung gian
                # Tỉ lệ chính xác hơn: số bước = góc * steps_per_deg
                dx = int(intermediate_theta1 * self.step_per_mm)
                dy = int(intermediate_theta2 * self.step_per_mm)
                
                # Gửi lệnh quay tuyệt đối thay vì tương đối
                self.send_command(f"GOTO {dx} {dy}")
                
                # Chờ một chút để robot thực hiện lệnh
                time.sleep(self.motor_delay)
            
            # Điều khiển servo nhấc bút sau khi đã di chuyển đến vị trí
            if pen == 1:
    ...
    def start_drawing(self):
        ...
        if self.use_gcode.get():
            ...
            self.drawing_thread = threading.Thread(target=self.gcode_simulation_process)
